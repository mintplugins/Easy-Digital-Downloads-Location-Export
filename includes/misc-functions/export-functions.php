...
class EDD_Payments_By_Location_Export extends EDD_Export {
	...
	public function get_data() {
		...
		foreach ( $payments as $payment ) {
			...
			if ( $payment_meta['country'] == $selected_country || $selected_country == 'all' ){
				
				//If a state/province has been selected
				if ( $payment_meta['state'] != '0' ){
					
					//Get all states in this country
					$country_states = edd_get_shop_states( $selected_country );
					
					//Loop through all states in this country
					foreach( $country_states as $state_code => $state_name ){
						
						//If the state code matched the selected state code
						if ( $selected_state == $state_code ){
							
							//Make the selected state value change from the code to the name
				...
				if ( $payment_meta['state'] == $selected_state || $selected_state == '0' ){
